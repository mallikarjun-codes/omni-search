...
import { Send, Bot, User, Sparkles, BookOpen, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-slate-900/40 rounded-2xl border border-white/5 overflow-hidden backdrop-blur-md">
      ...
      <div className="px-6 py-4 bg-white/5 border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-200">Interactive Session</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {messages.length === 0 ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-4 max-w-lg mx-auto">
            ...
            <h3 className="text-xl font-bold text-slate-100">AI Documentation Assistant</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
              {starterQuestions.map((q, idx) => (
                <button
                  ...
                  className="p-3 text-xs text-left text-slate-300 hover:text-white bg-white/5 hover:bg-white/10 border border-white/5 hover:border-brand-500/30 rounded-xl transition-all duration-300 glass-hover cursor-pointer"
                >
                  {q}
                </button>
              ))}
                        ...
                        {msg.sources.map((src, srcIdx) => {
                          ...
                          return (
                            <div key={srcIdx} className="bg-white/5 rounded-lg border border-white/5 overflow-hidden">
                              ...
                              >
                                <span className="font-medium truncate max-w-[80%]">📄 {src.docTitle}</span>
          ...
          <div className="flex gap-4 justify-start">
            ...
            <div className="glass-card text-slate-200 rounded-2xl rounded-tl-none px-5 py-4 flex flex-col gap-2">
              ...
              <div className="flex items-center gap-1.5 mt-1">
                ...
                <div className="w-2.5 h-2.5 rounded-full bg-brand-400 dot-bounce" style={{ animationDelay: '150ms' }}></div>
                <div className="w-2.5 h-2.5 rounded-full bg-brand-300 dot-bounce" style={{ animationDelay: '300ms' }}></div>
              </div>
            </div>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="p-4 bg-white/5 border-t border-white/5">
        <div className="relative flex items-center">
          <input
            ...
            className="w-full glass-input rounded-xl py-3.5 pl-4 pr-12 text-sm text-slate-100 placeholder-slate-500 focus:outline-none"
          />
          <button
            type="submit"
